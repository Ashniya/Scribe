      ...
      <div className="w-full px-6 sm:px-8 lg:px-12">
        <div className="flex justify-between items-center h-24">
          ...
          <div className="hidden md:flex items-center space-x-8 ml-auto">
            ...
            {/* Improved Theme Toggle - Better Visibility */}
            <button 
              onClick={handleThemeToggle}
              className="relative w-16 h-8 rounded-full bg-gradient-to-r from-gray-300 to-gray-400 dark:from-slate-700 dark:to-slate-600 p-1 transition-all duration-300 hover:shadow-lg group border border-gray-400 dark:border-slate-500"
            >
              <div className={`absolute w-6 h-6 rounded-full bg-white shadow-md transform transition-all duration-300 flex items-center justify-center ${isDark ? 'translate-x-8' : 'translate-x-0'}`}>
                {isDark ? (
              ...
              <div className="absolute inset-0 flex items-center justify-between px-2 pointer-events-none">
                <Sun size={12} className={`transition-opacity duration-300 ${isDark ? 'opacity-40 text-gray-200' : 'opacity-0'}`} />
                <Moon size={12} className={`transition-opacity duration-300 ${isDark ? 'opacity-0' : 'opacity-40 text-gray-600'}`} />
              </div>
            </button>
          </div>
          ...
          <div className="flex items-center space-x-4 md:hidden ml-auto">
            <button 
              onClick={handleThemeToggle}
              className="relative w-14 h-7 rounded-full bg-gradient-to-r from-gray-300 to-gray-400 dark:from-slate-700 dark:to-slate-600 p-1 transition-all duration-300 border border-gray-400 dark:border-slate-500"
            >
              <div className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-all duration-300 flex items-center justify-center ${isDark ? 'translate-x-7' : 'translate-x-0'}`}>
                {isDark ? (
        ...
        <div className="px-4 pt-2 pb-6 space-y-3 bg-scribe-cream/95 dark:bg-slate-900/95 backdrop-blur-md shadow-lg">
          ...
          {isAuthenticated ? (
            ...
            <>
              ...
              <Link to="/signup" className="block px-4 py-2.5 text-center bg-gradient-to-r from-scribe-green to-scribe-sage dark:from-scribe-mint dark:to-scribe-sage text-white dark:text-scribe-green rounded-lg font-medium">
                Get Started
              </Link>
            </>
          )}
