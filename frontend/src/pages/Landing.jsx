...
const Landing = () => {
  ...
  const writingExpRef = useRef(null);
  ...
  useEffect(() => {
    if (!showHero) {
      // Animate Writing Experience section
      gsap.fromTo(
        writingExpRef.current.querySelectorAll('.fade-in'),
        { opacity: 0, y: 60 },
        {
          opacity: 1,
          y: 0,
    ...
    <div className="min-h-screen bg-scribe-cream dark:bg-slate-900 transition-colors duration-500">
      ...
      {!showHero && (
        <>
          ...
          {/* New Writing Experience Section */}
          <section ref={writingExpRef} className="min-h-screen relative flex items-center justify-center py-32 bg-white dark:bg-slate-800 overflow-hidden">
            <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 z-10">
              <div className="grid md:grid-cols-2 gap-16 items-center">
                {/* Left: Content */}
                <div className="space-y-8 fade-in">
                  <h2 className="text-5xl md:text-6xl font-bold text-gray-800 dark:text-white leading-tight">
                    Experience the Joy of <span className="text-scribe-green dark:text-scribe-sage">Writing</span>
                  </h2>
                  <p className="text-xl text-gray-600 dark:text-gray-300 leading-relaxed">
                    Immerse yourself in a distraction-free environment designed for pure creativity.
                    Watch your ideas flow seamlessly from thought to words.
                  </p>
                  <p className="text-lg text-gray-500 dark:text-gray-400 leading-relaxed">
                    Every keystroke matters. Every sentence has purpose. This is where great stories begin.
                  </p>
                  <div className="flex flex-wrap gap-4 pt-4">
                    <div className="flex items-center space-x-2">
                      <div className="w-2 h-2 bg-scribe-green rounded-full"></div>
                      <span className="text-gray-700 dark:text-gray-300">Real-time auto-save</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="w-2 h-2 bg-scribe-sage rounded-full"></div>
                      <span className="text-gray-700 dark:text-gray-300">Focus mode</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="w-2 h-2 bg-scribe-mint rounded-full"></div>
                      <span className="text-gray-700 dark:text-gray-300">Beautiful typography</span>
                    </div>
                  </div>
                </div>

                {/* Right: Animated Typing Document */}
                <div className="flex justify-center items-center fade-in">
                  <div className="relative">
                    <svg width="400" height="500" viewBox="0 0 400 500" className="drop-shadow-2xl">
                      {/* Document Background */}
                      <rect x="40" y="20" width="320" height="460" fill={isDark ? "#1e293b" : "#ffffff"} rx="12" stroke={isDark ? "#9CAB84" : "#89986d"} strokeWidth="3" />

                      {/* Decorative Header */}
                      <rect x="40" y="20" width="320" height="60" fill={isDark ? "#334155" : "#f6f0d7"} rx="12" />
                      <circle cx="70" cy="50" r="8" fill={isDark ? "#9CAB84" : "#89986d"} opacity="0.6" />
                      <circle cx="95" cy="50" r="8" fill={isDark ? "#C5D89D" : "#9cab84"} opacity="0.6" />
                      <circle cx="120" cy="50" r="8" fill={isDark ? "#9CAB84" : "#c5d89d"} opacity="0.6" />

                      {/* Animated Typing Lines */}
                      <g className="typing-lines">
                        <line x1="80" y1="130" x2="80" y2="130" stroke={isDark ? "#9CAB84" : "#89986d"} strokeWidth="3" strokeLinecap="round" className="line-1">
                          <animate attributeName="x2" from="80" to="320" dur="2s" fill="freeze" repeatCount="indefinite" />
                        </line>
                        <line x1="80" y1="170" x2="80" y2="170" stroke={isDark ? "#9CAB84" : "#89986d"} strokeWidth="3" strokeLinecap="round" className="line-2">
                          <animate attributeName="x2" from="80" to="340" dur="2s" begin="0.5s" fill="freeze" repeatCount="indefinite" />
                        </line>
                        <line x1="80" y1="210" x2="80" y2="210" stroke={isDark ? "#C5D89D" : "#9cab84"} strokeWidth="3" strokeLinecap="round" className="line-3">
                          <animate attributeName="x2" from="80" to="280" dur="2s" begin="1s" fill="freeze" repeatCount="indefinite" />
                        </line>
                        <line x1="80" y1="250" x2="80" y2="250" stroke={isDark ? "#C5D89D" : "#9cab84"} strokeWidth="3" strokeLinecap="round" className="line-4">
                          <animate attributeName="x2" from="80" to="330" dur="2s" begin="1.5s" fill="freeze" repeatCount="indefinite" />
                        </line>

                        {/* Blinking Cursor */}
                        <rect x="80" y="290" width="3" height="24" fill={isDark ? "#9CAB84" : "#89986d"} className="cursor-blink">
                          <animate attributeName="opacity" values="1;0;1" dur="1s" repeatCount="indefinite" />
                          <animate attributeName="x" from="80" to="200" dur="2s" begin="2s" fill="freeze" repeatCount="indefinite" />
                        </rect>
                        <line x1="80" y1="302" x2="80" y2="302" stroke={isDark ? "#9CAB84" : "#89986d"} strokeWidth="3" strokeLinecap="round" className="line-5">
                          <animate attributeName="x2" from="80" to="200" dur="2s" begin="2s" fill="freeze" repeatCount="indefinite" />
                        </line>
                      </g>

                      {/* Decorative Elements */}
                      <circle cx="340" cy="450" r="12" fill={isDark ? "#C5D89D" : "#89986d"} opacity="0.3" />
                      <circle cx="320" cy="430" r="8" fill={isDark ? "#9CAB84" : "#9cab84"} opacity="0.3" />
                    </svg>
                    ...
                    <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
                      {[...Array(6)].map((_, i) => (
                        <div
                          key={i}
                          className="absolute w-1.5 h-1.5 bg-scribe-sage/40 dark:bg-scribe-sage/20 rounded-full animate-float"
                          style={{
